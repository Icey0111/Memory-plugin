...
export function fuseSettingCandidates(snapshotInput, {
    ...
    for (const row of byChunk.values()) {
        ...
        const current = byEntry.get(id) || {
            ...
            secret: Boolean(row.chunk.secret),
            known_by: [...(row.chunk.known_by || [])],
    ...
    for (const chunk of Array.isArray(snapshot.chunks) ? snapshot.chunks : []) {
        ...
        constantEntries.push({
            ...
            keys: [...(chunk.keys || [])],
            constant: true,
            secret: Boolean(chunk.secret),
            known_by: [...(chunk.known_by || [])],
            order: chunk.order,
...
function normalizeActorName(value) {
    return cleanText(value, 300).normalize('NFKC').toLocaleLowerCase();
}

/**
 * Explicit role-private setting visibility (Iteration 08).
 *
 * Imported entries are objective world reference data by default. An entry only becomes
 * role-private when the source explicitly marks it (`secret: true` / `visibility: secret`)
 * and names an audience (`known_by`). Nothing is inferred from prose or filenames.
 */
export function isSettingRowVisible(row, actorInput) {
    if (!row || !row.secret) return true;
    const actor = actorInput || {};
    const ids = (Array.isArray(actor.ids) ? actor.ids : []).map(value => cleanText(value, 300)).filter(Boolean);
    const aliases = (Array.isArray(actor.aliases) ? actor.aliases : []).map(normalizeActorName).filter(Boolean);
    const knownBy = uniqueStrings([...(row.known_by || []), ...(row.known_by_ids || [])], 64);
    if (!knownBy.length) return false;
    if (knownBy.some(id => ids.includes(id))) return true;
    return knownBy.some(name => aliases.includes(normalizeActorName(name)));
}

/** Drop role-private setting rows the active character must not see. */
export function filterSettingRowsForActor(retrievalInput, actorInput) {
    const retrieval = retrievalInput || {};
    const hidden = [];
    const keep = row => {
        if (isSettingRowVisible(row, actorInput)) return true;
        if (row?.entry_id) hidden.push(row.entry_id);
        return false;
    };
    return {
        ...retrieval,
        results: (Array.isArray(retrieval.results) ? retrieval.results : []).filter(keep),
        constant_entries: (Array.isArray(retrieval.constant_entries) ? retrieval.constant_entries : []).filter(keep),
        hidden_secret_entry_ids: uniqueStrings(hidden, 200),
    };
}
