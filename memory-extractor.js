...
export function normalizeExtractionOperation(op) {
    ...
    // Keep the value unless it is a genuinely unknown word. The channel vocabulary (saw/heard/told/
    // inferred) is the common wrong-field answer and memory-core resolves it; deleting it here would
    // throw away the only statement the model made about how certain the claim is.
    if (out.epistemic && !EPISTEMIC.includes(out.epistemic)
        && !['saw', 'seen', 'witnessed', 'heard', 'told', 'reported', 'inferred', 'inference', 'deduced', 'guessed', 'guess', 'suspected'].includes(String(out.epistemic).toLowerCase())) {
        delete out.epistemic;
    }
...
你的输出之后还会经过独立 Semantic Baseline 写入硬门；不要依赖硬门兜底，抽取阶段仍应主动避免基线重复。\n\n【证据原则】\noperations 必须能从最新用户消息或最新AI正文直接得到，或是对已有 active 状态的明确更新。\n必填 op.epistemic，取值只能是 fact（已确认的客观事实）/observed（亲眼所见）/reported（转述）/rumor（传闻）/belief（猜测或信念）/inference（推断）/plan（计划）。传闻、猜测、推断、计划分别写 rumor/belief/inference/plan，不得升级为 fact。op.channel 另写 saw/heard/told/inferred，表示“怎么知道的”；两个字段不要混用。\n必填 op.importance，取值只能是 critical（丢掉它后续剧情会直接矛盾且不可逆：承诺、所有权、关系、生死、身份）/high（会影响之后很多轮的判断或行动）/medium（只影响接下来一两轮）/low（一次性细节，下一轮不再需要）。不要把同一轮的 operations 全写成 medium；同一轮里 importance 不应完全相同，判断不了才用 medium。\nknowledge 类必须写 op.known_by：正文明确发生某角色得知或确认时，把真正知道这件事的角色名写进去；所有在场角色都知道、或正文没有划出认知边界时写空数组。known_by 是认知边界，写错会让不该知道的角色看到它。\n每条 operation 还要写 channel，表示该角色是「怎么知道的」：saw=亲眼所见或亲身经历，heard=在场听到他人说，told=事后被他人告知或转述，inferred=由已知信息推断。判断不了就留空。\n不要把模型因为看到系统提示/世界书而知道的事情，写成角色已经知道。\n\n【scope 适用范围】\n如果某条断言只在特定场景/条件/项目下成立（例如“学习数学时”“仅限终末星港”），在 op.scope 写这个适用范围；全局成立就留空。不要用 scope 表达时间点。\n\n【kind】\nevent / state / knowledge / belief / relation / commitment / ownership / intention / world_delta\n\n【op】\nadd / update / close / supersede / reinforce / invalidate / noop\n\n【slot 复用 — 强制】\n写入前必须先看上面【当前 Canonical 状态摘要】里已经存在的 slot。\n同一个主体的同一类信息，必须对已有 slot 用 update / supersede / close，**不得新建 slot**。\n只有当一个主题是全新的、并且以后还会被反复更新时，才 add 新 slot。\n可更新状态尽量给稳定 slot，例如“平成.state.hunger”“平成.location.current”“平成.intention.east_street_dinner”。\n状态结束时优先 close target_slot。新事实推翻旧 belief/knowledge 时使用 supersede。\n\n实测依据：上一版 223 个 add 中有 200 个新建了 slot，状态池膨胀到 190 条，固定注入预算最终只能覆盖 18%，记忆系统因此“越说越少”。同一主体同一 kind 的 slot 数量应保持在个位数；如果你正在为同一个人新建第 10 个 belief slot，那就应该 update 已有的那一个。\n\n【写入门槛 — 三条全过才写】\n1. 耐久性：这条信息三天后仍然成立吗？只是此刻的气氛、情绪或修辞理解，不写。\n2. 后果性：它会影响角色之后的行动、态度或判断吗？不影响的，不写。\n3. 可陈述性：能写成一句关于世界的事实吗（谁 / 在哪 / 知道什么 / 答应什么 / 拥有什么 / 什么发生了变化）？写不成，不写。\n\n【绝对不要写】\n对用户某句话的措辞分析；句子编号或计数（例如“这是第 21 句”）；修辞解读（例如“她把『我们』读成两个人”）；以及关于本轮对话本身的元评论。这些是这一轮的即时理解，不是世界状态。belief 只写角色对世界或他人的稳定判断。每轮 operations 通常不超过 6 条。\n\n【向量策略】\nindexable=true：值得未来语义召回的重要事件、关系、知识、承诺、重要失败/成功。\nindexable=false：当前状态、临时意图、低价值瞬时信息。\n不要为了凑数量创建记忆。\n\n【实体与主题】\nindexable=true 时尽量填写具体 entities/topics；text 必须自包含，使用真实名字，避免只写“她/那里/那个”。\n\n【当前 Canonical 状态摘要】\n${canonical || '（无）'}\n\n【最近上下文，仅用于消歧；不要把其中旧事件重新当成新事件】\n${recent || '（无）'}\n\n【与本轮变化相关的插件世界设定】
