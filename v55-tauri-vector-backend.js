...
async function requestEmbeddings(config,texts,role,fetchImpl) {
    ...
    const baseUrl=resolveOpenAiCompatibleBaseUrl(config.apiUrl), body=buildDirectEmbeddingBody({model:config.model,texts,apiUrl:baseUrl,role});
    let response; try { response=await fetchImpl(`${baseUrl}/embeddings`,{method:'POST',headers:{'Content-Type':'application/json',Accept:'application/json',Authorization:`Bearer ${apiKey}`},body:JSON.stringify(body)}); } catch(error) { throw new Error(`Embedding 网络请求失败：${String(error?.message||error)}。请检查网络/CORS/代理。`); }
    ...
    const payload=await response.json().catch(()=>null); if(!Array.isArray(payload?.data)) throw new Error('Embedding provider 返回格式无效：缺少 data 数组。');
    ...
    if(vectors.length!==texts.length) throw new Error(`Embedding provider 返回向量数量不匹配：期望 ${texts.length}，实际 ${vectors.length}。`); return vectors;
}
...
export async function handleTauriVectorRequest(endpoint,payloadInput,configInput,fetchImpl=globalThis.fetch?.bind(globalThis)){if(!isNativeTauriTavern())return null;if(typeof fetchImpl!=='function')throw new Error('Direct Embedding fetch 不可用。');const payload=payloadInput&&typeof payloadInput==='object'?payloadInput:{},config=configInput&&typeof configInput==='object'?configInput:{};if(!clean(payload.collectionId,4000))throw new Error('Tauri vector request 缺少 collectionId。');if(['insert','query'].includes(endpoint)&&(!clean(config.apiUrl,4000)||!clean(config.model,1000)))throw new Error('Tauri direct Embedding transport 配置不完整。');switch(endpoint){case'insert':return insertCollection(payload,config,fetchImpl);case'query':return queryCollection(payload,config,fetchImpl);case'list':return listCollection(payload);case'delete':return deleteItems(payload);case'purge':return purgeCollection(payload);default:throw new Error(`Tauri plugin vector backend 不支持 endpoint: ${endpoint}`);}}
export function getTauriVectorBackendStatus(){return{active:isNativeTauriTavern(),backend:'tauritavern-plugin-vector-v2',persistence:'window.__TAURITAVERN__.api.extension.store',credential_scope:'aetheria-localStorage',has_api_key:Boolean(getTauriVectorApiKey()),cached_collection_count:collectionCache.size,last_debug:lastDebug?structuredClone(lastDebug):null};}
