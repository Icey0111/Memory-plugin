# Aetheria Unified Memory v5.5-dev — Architecture
...
## 18. Iteration 07/08 boundary — hardening and release-blocking fixes

Iteration 07 added chat → world/revision binding, a chat-local entity registry, branch provenance,
Canonical Current State authority, a knowledge-visibility filter, the scene-summary lifecycle, the
unified Reference + Current State budget and the Setting Entry overlay editor. Two defects shipped
with it and are fixed in Iteration 08:

1. `v55-consistency.js` stamped runtime identity before stabilizing provenance, so the first-observed
   branch was overwritten. Provenance now runs first (`stabilizeProvenanceStore` → `stampRuntimeIdentity`).
2. `v55-finalizer.js` / `v55-consistency.js` re-injected the Scene Summary block after the legacy
   interceptor had cleared both prompt keys for quiet / impersonate / disabled generations. They now
   forward the cleared payloads and skip post-processing for suppressed generations. The plugin's own
   background extraction is always suppressed; third-party quiet injection is opt-in via
   `quiet_allow_third_party_injection` and never applies to the plugin's own quiet call.

Iteration 08 also implements the remaining proposal items:

- explicit role-private setting schema (`secret` / `visibility` / `known_by`) filtered by the active
  actor for generation and extraction prompts; dedup still sees the full objective index;
- condition/exception-aware chunking (`splitSettingText`) and same-name entity disambiguation
  (`entity_scope` / `entity_keys`, explicit discriminator never falls back);
- baseline write-gate calibration samples + entity-disjoint guard + extra change markers;
