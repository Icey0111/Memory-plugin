...
export function stampRuntimeIdentity(ctx) {
    ...
    for (const memory of Object.values(memories)) {
        ...
        for (const alias of unique(memory.entities)) {
            let id = resolveEntityId(registry, alias, alias === charName ? charDiscriminator : '');
            if (!id) id = registerEntity(registry, binding.world_id, alias, '', 'story-entity');
            if (id) entityIds.push(id);
        }
        const knownByIds = [];
        for (const alias of unique(memory.known_by)) {
            let id = resolveEntityId(registry, alias, alias === charName ? charDiscriminator : (alias === userName ? 'st-user' : ''));
            if (!id) id = registerEntity(registry, binding.world_id, alias, '', 'story-entity');
