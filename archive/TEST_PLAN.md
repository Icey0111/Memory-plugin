# Aetheria Unified Memory v5.5-dev — Test Plan
...
## Iteration 08 additions

`npm run check` covers the same 18 modules. `npm test` now runs 33 scripts and must pass 33/33:

- `test-v55-fullstack-cleanup.mjs` — real `index-v55.js` stack with extractions: quiet / impersonate /
  disabled stay cleared while normal keeps Reference + Current State; opt-in third-party quiet injects
  while the plugin's own extraction stays cleared.
- `test-v55-entity-identity.mjs` — same-name characters with different `entity_scope` do not share an
  entity id; host character/user use their host discriminator; same scope reuses one id.
- `test-v55-scene-evidence.mjs` — bounded, derived-only scene evidence expansion with summary fallback.
- `test-setting-secret-visibility.mjs` — explicit secret schema only, audience filtered by id or alias,
  constants included, no-audience secrets hidden from everyone.
- `test-setting-chunk-conditions.mjs` — conditional/exception clauses stay with their rule; unrelated
  long sentences still split.
- `test-baseline-calibration.mjs` — the five proposal calibration scenarios plus a story-delta
  false-reject-rate assertion.

Known remaining external work: Commit H real SillyTavern final-request inspection, provider System
handling, and real-provider threshold calibration.
