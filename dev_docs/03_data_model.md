...
| narrative_anchors | { version, active, superseded, resolved, parse, updated_at } | updateNarrative | Promises, ownership, secrets and life states, re-injected until something explicitly resolves them. An active entry is `{ id, kind, subject, text, first_seen, last_confirmed, passes, unconfirmed }`; `subject` is the stable identity that lets a newer value supersede an older one, and `superseded` is a bounded history of what a newer statement replaced (ADR-0026). Identity is `anchorSubjectKey`: Unicode-normalised, whitespace-collapsed and with a `/` suffix dropped - deterministic by decision, because a containment rule was measured merging five genuinely different subjects to catch one duplicate (ADR-0027) |
