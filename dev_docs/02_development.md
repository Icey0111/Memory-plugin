...
<!-- VERSION 3 -->
## v3 - 2026-09-12 18:49:43 - document the paraphrase set, which is the instrument the dense decision waits on
...
<!-- VERSION 5 -->
## v5 - 2026-09-12 21:05:40 - document the budget and slot switches, which are how the frontier was measured
...
--evidence <tokens> and --entries <n> pin the evidence budget and the slot count, which is how the
frontier in dev_docs/06_retrieval_research.md section 12 was measured. With no --entries the ruler uses
the slot count the budget pays for, which is what the plugin does.
...
<!-- VERSION 6 -->
## v6 - 2026-09-12 21:18:58 - document the vector cache and the dense switches the ruler grew
...
--evidence <tokens> and --entries <n> pin the evidence budget and the slot count, which is how the
frontier in dev_docs/06_retrieval_research.md section 12 was measured. With no --entries the ruler uses
the slot count the budget pays for, which is what the plugin does. --embeddings <cache> --dense-weight <w>
run the dense channel, and recall-embed.mjs builds that cache from an OpenAI-compatible endpoint with the
same chunk text and retrieval task the plugin embeds with:

    node recall-embed.mjs <chat.jsonl> --questions <set.json> --out remove/_emb.json --key-file <path>

The key comes from the file or EMBEDDINGS_API_KEY and is never printed or written; an existing cache is
topped up rather than rebuilt. The dense A/B in dev_docs/06_retrieval_research.md section 13 is that
command plus the two ruler flags.
the slot count the budget pays for, which is what the plugin does.
...
1. npm run check and npm test pass (69 source files, 32 test files).
2. The narrative acceptance test still passes: a detail that exists only in the original text is
   recallable, and no floor is hidden without a stand-in.
3. The change is recorded in change_log/ with the four required sections, and any durable decision
   is recorded as an ADR in dev_docs/decisions/.
...
<!-- VERSION 7 -->
## v7 - 2026-09-12 21:51:03 - document the rerank stage and the two question sets the layers were decided on
...
run the dense channel, and recall-embed.mjs builds that cache from an OpenAI-compatible endpoint with the
same chunk text and retrieval task the plugin embeds with:
the dense A/B in dev_docs/06_retrieval_research.md section 13 is that command plus the two ruler flags.

A rerank stage is wired into the pipeline behind the narrative_rerank_model setting. Empty means off,
which is what an install that has not configured one keeps; when it is set the fused shortlist is
reranked once per generation and any failure leaves the fused order in place and records the reason in
the diagnostics. v55-rerank.js is the transport and is tested with a fake fetch.

Two question sets exist, and the layer decisions were made on both: remove/_paraphrases2.json (52
questions on a chat with no capsule) and remove/_paraphrases3.json (40 questions on the 40-floor chat,
the only one carrying a summary and anchors). Neither ships; the format does.
...
1. npm run check and npm test pass (70 source files, 32 test files).
2. The narrative acceptance test still passes: a detail that exists only in the original text is
   recallable, and no floor is hidden without a stand-in.
3. The change is recorded in change_log/ with the four required sections, and any durable decision
   is recorded as an ADR in dev_docs/decisions/.
