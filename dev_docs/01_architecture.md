...
| May this floor leave the prompt? | Only if every chunk of it is covered by a committed batch - a complete turn, or the opening greeting covered together with one (ADR-0047) | raw-history.js, applyNarrativeFolds |
...
| N2 | Each accepted batch hides exactly N complete turns, including its last turn, plus the greeting once one turn is covered; batch-external text stays visible (ADR-0023, ADR-0047) | nextSummaryBatch, applyNarrativeFolds |
...
| N41 | A quoted window follows the region the channel that picked the span actually found: a trimmed span's incumbent seats are the character channel's densest descriptor run and the rare-term channel's term occurrence (a term's seat reaching back 48 characters, a character's own name never a seat), with the head as fallback, and the window terms are themselves filtered by story frequency; a character's introduction row is a second candidate of the character channel unless another candidate name is mentioned earlier in that same row, and one descriptor cluster is the *densest* 120-character run near a mention rather than the count in a wide one (ADR-0045) | descriptorCluster, profileTargets, entityTargets, rankRawChunks, packRawEvidence, queryWindowTerms, test-profile-window.mjs |
