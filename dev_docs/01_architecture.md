...
| N34 | A trimmed evidence quote keeps the part of the message the question is about: the window that covers the most of the question's own terms wins, the head-anchored window is the incumbent, only strictly more coverage moves it, and the length, budget and slot count are unchanged (ADR-0037) | fitEvidenceSpan, slideWindowToQuery, packRawEvidence, test-evidence-window.mjs |
