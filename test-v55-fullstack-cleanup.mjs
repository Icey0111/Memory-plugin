...
assert.ok(normal.ref); assert.doesNotMatch(normal.ref[1],/SCENE SUMMARY LOCATORS/,'the duplicated locator block is not injected'); assert.match(normal.ref[1],/公开长期摘要/); assert.ok(normal.cur); assert.match(normal.cur[1],/测试大厅/); assert.equal(normal.summary?.[1],''); assert.equal(normal.summary?.[3],0);
for(const type of['quiet','impersonate']){const row=await run(type);assert.equal(row.ref?.[1],'');assert.equal(row.cur?.[1],'');assert.equal(row.summary?.[1],'');}
settings.quiet_allow_third_party_injection=true; assert.ok((await run('quiet')).ref?.[1]); settings.__quiet_extraction_in_progress=true; const ownQuiet=await run('quiet'); assert.equal(ownQuiet.ref?.[1],''); assert.equal(ownQuiet.summary?.[1],''); delete settings.__quiet_extraction_in_progress; settings.quiet_allow_third_party_injection=false;
settings.enabled=false; const disabled=await run('normal'); assert.equal(disabled.ref?.[1],''); assert.equal(disabled.cur?.[1],''); assert.equal(disabled.summary?.[1],''); assert.equal(disabled.legacy?.[1],'');
